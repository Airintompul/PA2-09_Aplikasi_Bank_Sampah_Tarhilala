...
<template>
    <AdminLayout>
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-4xl font-black text-gray-900 uppercase tracking-tight">Reward</h2>
        ...
        <div v-if="successMessage" class="mb-6 p-5 bg-green-500 text-white rounded-2xl font-bold shadow-lg flex items-center">
            <i class="fa-solid fa-gift mr-3"></i> {{ successMessage }}
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100 text-black font-medium">
                    ...
                    <tr v-for="reward in rewards" :key="reward.id" class="hover:bg-gray-50/50 transition-all">
                        ...
                        <td class="px-6 py-6 font-black text-lg uppercase text-gray-800">{{ reward.nama_reward }}</td>
                        <td class="px-6 py-6 text-center font-bold text-blue-600">{{ Number(reward.poin_dibutuhkan).toLocaleString() }} Poin</td>
                        <td class="px-6 py-6 text-center font-bold">{{ reward.stok }}</td>
                        <td class="px-6 py-6 text-center uppercase">
                            <span :class="reward.stok > 0 ? 'text-green-600' : 'text-red-600'" class="font-black text-xs">
                                {{ reward.stok > 0 ? 'Tersedia' : 'Habis' }}
                            </span>
                        </td>
                        <td class="px-8 py-6 text-center relative">
                            <div class="flex items-center justify-center space-x-2">
                                <button @click="openEditModal(reward)" class="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button @click="currReward = reward; openDelete = true" class="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all">
        ...
        <div v-if="openAdd || openEdit" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div class="bg-white rounded-[3rem] max-w-lg w-full p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                <h3 class="text-2xl font-black text-gray-800 uppercase tracking-tighter mb-8">
                    {{ openAdd ? 'Add New Reward' : 'Update Reward' }}
                </h3>

                <form @submit.prevent="openAdd ? handleStore() : handleUpdate()" class="space-y-4">
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Nama Reward</label>
                        <input v-model="currReward.nama_reward" type="text" required class="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-[#41D3BD] outline-none font-bold text-gray-700">
                    </div>
        ...
        <div v-if="openDelete" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div class="bg-white rounded-[3rem] max-w-sm w-full p-10 text-center shadow-2xl">
                ...
                <h3 class="text-xl font-black text-gray-800 uppercase tracking-tight mb-2">Hapus Reward?</h3>
                <p class="text-gray-400 text-sm font-bold mb-8 uppercase tracking-tighter">Menghapus <span class="text-red-500">{{ currReward.nama_reward }}</span> tidak dapat dibatalkan.</p>
